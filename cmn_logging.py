...
class CmnLogger(logging.getLoggerClass()):
    ...
    def check(self, condition, *args, **kwargs): # pylint: disable=missing-docstring
        if condition:
            return True
        if not (len(args) or "msg" in kwargs):
            kwargs['msg'] = "check failed!"
...
def build_logger(name, level=logging.INFO, use_color=False, filehandler=None, append_log=False):
    """Create a logger, console handler and formatter.
    Do this in one function to allow for more uniformity across tools.
    If append_log is True and filehandler is not None, the FileHandler for this logger appends to the
    previous logfile. Otherwise the previous logfile is deleted and starts clean.
    """
    ...
    if filehandler:
        ...
        if not append_log:
            mode = 'w'
        fhandler = logging.FileHandler(filehandler, mode=mode)
